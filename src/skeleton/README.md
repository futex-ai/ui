...
pulsing bar and circle, a row/column group, and the pulse provider that lets a
whole skeleton breathe off one animation.
...
- Breathe with an opacity pulse driven by React Native's `Animated` API — not a
  CSS gradient sweep — so the same component animates identically on iOS,
  Android, and web, and stop the loop on unmount.
- Share one pulse across a whole skeleton via `SkeletonGroup` /
  `SkeletonPulseProvider`, so a grouped skeleton runs a single animation and its
  placeholders breathe in unison.
- Honour `prefers-reduced-motion`: render a clear, static placeholder instead of
  an animated pulse (WCAG 2.1 — 2.3.3 AAA).
...
- Use shared theme colours (the decorative `border2` neutral) instead of
...
Placeholders pulse on their own when used standalone. Wrap them in a
`SkeletonGroup` (which also lays them out) or a bare `SkeletonPulseProvider` to
drive every descendant from a single loop, so they breathe in unison and the
skeleton runs one animation rather than one per element.

## Theming

Placeholders read their fill from `SharedUiThemeProvider`: the `border2`
decorative neutral, the same token the `Spinner` track uses. The corner radius
resolves against `theme.radii` when given a token (`sm` / `md` / `lg` / `pill` /
`xl` / `xxl`).

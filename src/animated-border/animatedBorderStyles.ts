...
/**
 * Resolve the box `width` / `height`, `borderRadius`, and `borderWidth` into the
 * stroked-rect geometry. The stroke is centered on the rect edge, so the rect is
 * inset by half the stroke width on every side and its radius pulled in to
 * match. The radius is also clamped to half the shorter side — exactly what SVG
 ...
 * (a pill). The perimeter is the four straight edges (each shortened by a corner
 * radius at both ends) plus the four quarter-circle corners, which together make
 * one full circle of circumference `2πr` — so the same formula holds for a
 * square badge or a wide pill.
 */

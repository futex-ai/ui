...
- [Sortable group coordinator](sortable-group-coordinator.md) — M1–M2 delivered: the drag engine is generalised to N groups behind the unchanged single-list API, and a `SortableGroups` provider that lets several `SortableList`s exchange items by pointer and keyboard, reporting `{ key, fromGroupId, toGroupId, fromIndex, toIndex }` through one `onMove`, with a `groupFlow` keyboard model. `npm run verify` green; M3 remaining (`canDrop`, axe pass, docs).

...
- [Sortable group coordinator](sortable-group-coordinator.md) — M1 delivered (drag engine generalised to N groups behind the unchanged single-list API, shared portaled drag ghost, new browser coverage) and `npm run verify` green; M2–M3 remaining: a `SortableGroups` provider that lets several `SortableList`s exchange items by pointer and keyboard, reporting `{ key, fromGroupId, toGroupId, fromIndex, toIndex }` through one `onMove`, with `canDrop` and a `groupFlow` keyboard model. The single-list API is preserved; the drag engine is generalised to N groups rather than copied. M1–M3.

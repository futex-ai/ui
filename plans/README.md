...
- [Sortable group coordinator](sortable-group-coordinator.md) — not started: a `SortableGroups` provider that lets several `SortableList`s exchange items by pointer and keyboard, reporting `{ key, fromGroupId, toGroupId, fromIndex, toIndex }` through one `onMove`, with `canDrop` and a `groupFlow` keyboard model. The single-list API is preserved; the drag engine is generalised to N groups rather than copied. M1–M3.

...
import { accessibilitySweepShards, storiesForShard } from "./a11ySharding";
 ...
 * stories are scanned automatically with no list to maintain. Normal checks split
 * the sorted list into four deterministic shards that Playwright can schedule in
 * parallel. Each story is rendered in isolation (`/iframe.html?id=…`) in its own
 * page and scanned with axe-core against the WCAG 2.0/2.1 A and AA rule tags. A
 * fresh page per story keeps memory flat and isolates a crash to a single story.
 *
...
const sweepTimeout = updateBaseline ? 1_800_000 : 600_000;
...
test.describe("Storybook WCAG 2.1 A/AA axe sweep", () => {
  test.describe.configure({
    mode: updateBaseline ? "serial" : "parallel",
  });

  for (const shard of accessibilitySweepShards(updateBaseline)) {
    const shardLabel =
      shard.total === 1
        ? "all stories"
        : `shard ${shard.index + 1}/${shard.total}`;

    test(`has no new violations (${shardLabel})`, async ({
      browser,
      request,
      baseURL,
    }) => {
      ...
      expect(stories.length, "Storybook should expose stories").toBeGreaterThan(
        0,
      );
      ...
      for (const story of storiesForShard(stories, shard)) {
        ...
        try {
          ...
          const ruleIds = [
            ...new Set(results.violations.map((violation) => violation.id)),
          ].sort();
          if (ruleIds.length > 0) {

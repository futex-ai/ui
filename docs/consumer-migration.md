...
switch, modal, and avatar components copied into this package. The migration
...
8. Replace the `Avatar` export from `@/components/ui` with `@futex/ui/avatar`
...
- Existing dropdown, segmented control, radio card, switch, modal, and avatar
...
3. Import dropdowns from `@futex/ui/dropdown`, radio cards from
   ...
   from `@futex/ui/switch`, avatars from `@futex/ui/avatar`, and web modals from
   `@futex/ui/modal`.
